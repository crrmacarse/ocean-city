/**
 * Fetches channels of current user
 * This should be preloaded on first load.
 * There are three 4 types of channel: public, private, im, mpim
 */
export const FETCH_CHANNELS_REQUEST = 'FETCH_CHANNELS_REQUEST';
export const FETCH_CHANNELS_SUCCESS = 'FETCH_CHANNELS_SUCCESS';
export const FETCH_CHANNELS_FAILED = 'FETCH_CHANNELS_FAILED';

export const FETCH_ACTIVE_CHANNELS_REQUEST = 'FETCH_ACTIVE_CHANNELS_REQUEST';
export const FETCH_ACTIVE_CHANNELS_SUCCESS = 'FETCH_ACTIVE_CHANNELS_SUCCESS';
export const FETCH_ACTIVE_CHANNELS_FAILED = 'FETCH_ACTIVE_CHANNELS_FAILED';

export const SET_OPENED_CHANNEL = 'SET_OPENED_CHANNEL';
export const ADD_TO_ACTIVE_CHANNEL = 'ADD_TO_ACTIVE_CHANNEL';
export const CLOSE_CHANNEL = 'CLOSE_CHANNEL';

// fetches all messages of a channel
export const FETCH_MESSAGES_REQUEST = 'FETCH_MESSAGES_REQUEST';
export const FETCH_MESSAGES_SUCCESS = 'FETCH_MESSAGES_SUCCESS';
export const FETCH_MESSAGES_FAILED = 'FETCH_MESSAGES_FAILED';

// pushes a message to a channel object
export const PUSH_NEW_MESSAGE = 'PUSH_NEW_MESSAGE';

// read a message
export const SET_READ_MESSAGE = 'SET_READ_MESSAGE';

export const SEND_MESSAGE_REQUEST = 'SEND_MESSAGE_REQUEST';
export const SEND_MESSAGE_SUCCESS = 'SEND_MESSAGE_SUCCESS';
export const SEND_MESSAGE_FAILED = 'SEND_MESSAGE_FAILED';

export const FETCH_MASTER_LIST_REQUEST = 'FETCH_MASTER_LIST_REQUEST';
export const FETCH_MASTER_LIST_SUCCESS = 'FETCH_MASTER_LIST_SUCCESS';
export const FETCH_MASTER_LIST_FAILED = 'FETCH_MASTER_LIST_FAILED';

export const SET_USER_LIST = 'SET_USER_LIST';

export const CHECK_USER_PRESENCE_REQUEST = 'CHECK_USER_PRESENCE_REQUEST';
export const CHECK_USER_PRESENCE_SUCCESS = 'CHECK_USER_PRESENCE_SUCCESS';
export const CHECK_USER_PRESENCE_FAILED = 'CHECK_USER_PRESENCE_FAILED';

export const SET_USER_PRESENCE = 'SET_USER_PRESENCE';

export const FETCH_THREAD_REQUEST = 'FETCH_THREAD_REQUEST';
export const FETCH_THREAD_SUCCESS = 'FETCH_THREAD_SUCCESS';
export const FETCH_THREAD_FAILED = 'FETCH_THREAD_FAILED';

export const PUSH_THREAD_MESSAGE = 'PUSH_THREAD_MESSAGE';

export const PUSH_CHANNEL = 'PUSH_CHANNEL';
